...
- `POST {url}/manage/alias-conflicts` — **用户名冲突清单**（需登录；请求体可留空）。返回 `count` 与 `conflicts[]`，每条含冲突名字 `name`、在用档案 `current`、记为曾用名的档案 `owner`（各带 `name`/`uuid`/`from`/`lastLogin`）。只读内存索引、不读盘、不访问上游，**仅供展示**
...
- **🔍 缓存查询**：只读本地档案（`/manage/query`），**不会访问任何上游**；页面打开与切换都不会触发远程请求。按**玩家名**查询（缓存文件就是按名字存的）：输入 UUID 会明确提示"只支持玩家名"，按 UUID 请改用「远程信息」或「UUID 互转换」。
...
- **♻ 检查/修复 UUID 表**（玩家列表与概览页按钮）：先体检并列出幽灵条目、重复 uuid、缺失映射，再按需一键修复。曾用名冲突**不算索引问题**，不在这里显示（见下一条）。
- **⚠ 用户名冲突**：列出"某个名字既是在用档案的当前名、又被另一个档案记为曾用名"的情况。通常是名字被回收（原主人改名）后又被别人注册，属正常现象，**只影响「曾用名追踪」页的展示，不影响登录与改名判定**。本页**只展示、不提供修复**——确认某条是脏数据时，到「缓存修改」页把那份档案的 `old_names` 手工删掉再回来刷新即可；每张卡片提供「查看在用档案 / 查看曾用名档案 / 用该名字反查」三个跳转。启动日志里同一冲突只按缓存目录汇总一行（`[UUID_CACHE] Alias conflicts in <目录>: N`），逐条明细设 `debug: true` 才会打印。
